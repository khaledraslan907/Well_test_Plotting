...
if selected_features and not filtered.empty:
    ...
    def dataframe_to_excel_bytes(raw_df, filtered_df):
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine="xlsxwriter", datetime_format="yyyy-mm-dd hh:mm") as writer:
            raw_df.to_excel(writer, sheet_name="All_Detected_Data", index=False)
            filtered_df.to_excel(writer, sheet_name="Filtered_Data", index=False)
            workbook = writer.book

            for sheet_name, df in [("All_Detected_Data", raw_df), ("Filtered_Data", filtered_df)]:
    ...
    with d1:
        st.download_button(
            ...
            data=dataframe_to_excel_bytes(data, filtered),
            file_name="tmu_cleaned_results.xlsx",
    ...
    with d2:
        st.download_button(
            ...
            file_name="tmu_filtered_results.csv",
            mime="text/csv",
        )
