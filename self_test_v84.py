...
# Production history: one arithmetic mean per test, no second trend series.
rows = []
...
history = build_production_history(source, ["gross_rate_bpd", "oil_rate_stbd"])
assert len(history) == 3
assert history["test_id"].nunique() == 1
expected_first = source[source["test_id"] == "TEST-1"]["gross_rate_bpd"].mean()
assert abs(float(history.iloc[0]["gross_rate_bpd"]) - float(expected_first)) < 1e-9
assert history.iloc[0]["history_method"] == "Average of all valid readings in each test"
assert not any(str(c).startswith("_history_trend_") for c in history.columns)

# Every-20 default for 54 test points: first, 20th-step points, and last.
label_indices = set([0, 53] + list(range(0, 54, 20)))
assert label_indices == {0, 20, 40, 53}

print("Production Test Dashboard v84 self-test: PASS")
