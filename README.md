...
- Stronger fix for Streamlit Cloud Excel parsing differences.
- Adds `safe_text()` and `safe_join()` across parser header/text joins.
- Reads Excel as object dtype to reduce pandas/openpyxl environment differences.
- Shows `Parser build: v19` in the app so you can confirm Streamlit is actually running the updated code.
- Shows technical traceback details in the app if a file still fails, so the exact line can be identified.
