...
def row_keywords_score(row: pd.Series) -> int:
    txt = " ".join(normalize_text(v) for v in row.tolist())
...
def row_looks_like_units(row: pd.Series) -> bool:
    ...
    if not vals:
        return False
    txt = " ".join(normalize_text(v) for v in vals)
...
def header_score(row: pd.Series) -> float:
    ...
    row_txt = " ".join(normalize_text(v) for v in row.tolist())
...
def detect_header_row(raw_df: pd.DataFrame, max_scan_rows: int = 80) -> int:
    ...
    for i in range(scan_rows):
        ...
        group_has_date_time = "date" in " ".join(normalize_text(v) for v in raw_df.iloc[i].tolist()) or "hh:mm" in " ".join(normalize_text(v) for v in raw_df.iloc[i].tolist())
        header_rows = 1
        ...
        for r in range(i + 1, min(len(raw_df), i + 6)):
            ...
            if is_header_like_subrow(raw_df.iloc[r]) or row_looks_like_units(raw_df.iloc[r]) or header_score(raw_df.iloc[r]) > 8:
                ...
                header_rows += 1
                row_txt = " ".join(normalize_text(v) for v in raw_df.iloc[r].tolist())
...
def extract_well_from_raw(raw_df: pd.DataFrame, source_name: str = "", sheet_name: str = "") -> Optional[str]:
    ...
    try:
        ...
        text_blob = " ".join(
            ...
            if not pd.isna(raw_df.iloc[r, c])
        )
...
def standardize_dataframe(
    ...
    if row_filter_cols:
        ...
        raw_text_for_row = df.astype(str).apply(lambda r: " | ".join(r.values), axis=1).str.lower()
