...
ENGINE_ID = "v73-adaptive-semantic-table-engine-20260626"
...
def infer_field(header: object) -> FieldGuess:
    ...
    if re.search(r"\b(?:n2|nitrogen)\b", h) and re.search(r"\b(?:rate|flow|standard)\b", h):
        unit = _gas_unit(h)
...
def convert_series(values: pd.Series, guess: FieldGuess) -> pd.Series:
    out = values.map(number).astype("float64")
    unit = guess.unit
    key = guess.key or ""
...
def read_xlsx_sheets(data: bytes, max_rows: int = 50000, max_cols: int = 768,
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        ...
        for sheet_name, path in _sheet_paths(zf):
            ...
            raw = pd.DataFrame([[rows[r].get(c) for c in range(1, width + 1)] for r in row_nums],
...
def _combined_headers(raw: pd.DataFrame, start: int, height: int) -> List[str]:
    ...
    for c in range(width):
        val = safe_text(block[0][c])
        if val:
            current = val
        top_ffill.append(current)
    headers = []
    for c in range(width):
        parts: List[str] = []
        seen = set()
...
def _numeric_ratio(series: pd.Series) -> float:
    ...
    return 0.0 if sample.empty else sample.map(number).notna().mean()
...
def find_candidates(raw: pd.DataFrame) -> List[Candidate]:
    ...
    for start in range(scan):
        for height in (1, 2, 3, 4):
            ...
            guesses = [infer_field(h) for h in headers]
            ...
            # Avoid expensive content profiling on every ordinary data row.
            # Content-only timing discovery is reserved for the top of a sheet;
            # normal candidates must advertise Date/Time/Timestamp in the header.
            has_timing_hint = bool(guessed_keys & {"datetime", "date", "time"}) or bool(re.search(r"\b(?:date|time|timestamp|hh:mm)\b", header_joined))
            if not has_timing_hint and not (start < 12 and height == 1):
                continue
            dt_mode, dt_score = _detect_dt_mode(raw, start + height, guesses)
            if dt_mode[0] == "none":
                continue
            sample = raw.iloc[start + height:min(len(raw), start + height + 50)]
            num_cols = sum(_numeric_ratio(sample.iloc[:, i]) >= 0.45 for i in range(sample.shape[1]))
            if not known and num_cols < 2:
                continue
            unit_bonus = sum(bool(re.search(r"psi|bar|mmscf|scf|bbl|stb|ppm|%|hz|lbf|spm|/64", normalize(h))) for h in headers)
...
def parse_candidate(raw: pd.DataFrame, candidate: Candidate, source_name: str, sheet_name: str) -> pd.DataFrame:
    ...
    # Stop after the table has started and a long blank/non-data section begins.
    numeric_profiles = [_numeric_ratio(data.iloc[:, i]) for i in range(data.shape[1])]
    measurement_cols = [i for i, ratio in enumerate(numeric_profiles) if ratio >= 0.25 and i not in {candidate.dt_mode[1], candidate.dt_mode[2]}]
    valid_measurements = pd.Series(False, index=data.index)
    for i in measurement_cols:
        valid_measurements |= data.iloc[:, i].map(number).notna()
    ...
    for i, (header, guess) in enumerate(zip(candidate.headers, candidate.guesses)):
        ...
        if guess.key:
            ...
            if prev and prev[1] >= guess.confidence:
                ...
                out[raw_key] = data.iloc[:, i].map(number)
            ...
            out[key] = data.iloc[:, i].map(number)
            unknown_headers.append(header)
